...
class DataCombiner:
    def __init__(self, key_column: str, always_create_parent: bool = True, show_progress: bool = True):
    ...
    def combine_from_prefix(self, directory: str, prefix: str, output_filename: str, use_filename_as_column_name: bool = False) -> pd.DataFrame:
        files = sorted(glob.glob(str(Path(directory) / f"{prefix}*.csv")))
        dfs, warnings = [], []
        with ThreadPoolExecutor() as executor:
            futures = {executor.submit(
                self._load_and_prepare, f, prefix, use_filename_as_column_name): f for f in files}
        ...
        merged = reduce(self._merge_two, dfs)
    ...
    def combine_files(self, file_prefix_pairs: list[tuple[str, str]], output_filename: str) -> pd.DataFrame:
        if not file_prefix_pairs:
            raise ValueError("No file-prefix pairs provided")
        with ThreadPoolExecutor() as executor:
            futures = {executor.submit(self._prefix_and_load, file, prefix): (
                file, prefix) for file, prefix in file_prefix_pairs}
            loader = as_completed(futures)
            if self.show_progress:
                loader = tqdm(loader, total=len(futures), desc="Loading files")
            dfs = [future.result() for future in loader]
        merged = reduce(self._merge_two, dfs)
        self.result_df = merged
        out_path = Path(output_filename)
        if out_path.suffix.lower() != ".csv":
